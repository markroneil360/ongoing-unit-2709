...
OUT = ROOT / "data" / "semester_summary_fast.json"
...
def semester_rollups(data: dict[int, str]):
    groups = {}
    for epoch, band in data.items():
        dt = datetime.fromtimestamp(epoch, timezone.utc).astimezone(TZ)
        if (dt.year, dt.month, dt.day) < (2026, 4, 12):
            continue
        sem = 1 if dt.day <= 15 else 2
        key = f"{dt.year:04d}-{dt.month:02d}-S{sem}"
        groups.setdefault(key, {})[epoch] = band

    rows = []
    for key in sorted(groups):
        period = groups[key]
        stats, _runs = summarize(period)
        first = datetime.fromtimestamp(min(period), timezone.utc).astimezone(TZ)
        last = datetime.fromtimestamp(max(period), timezone.utc).astimezone(TZ)
        sem = 1 if key.endswith("-S1") else 2
        start_day = 1 if sem == 1 else 16
        end_day = 15 if sem == 1 else calendar.monthrange(first.year, first.month)[1]
        rows.append({
            "key": key,
            "label": f"{first.strftime('%B %Y')} — Infrasound Semester {sem}",
            "calendar_window_et": f"{first.strftime('%B')} {start_day}–{end_day}, {first.year}",
            "observed_first_minute_et": first.strftime("%Y-%m-%d %I:%M %p %Z"),
            "observed_last_minute_et": last.strftime("%Y-%m-%d %I:%M %p %Z"),
            "partial": key == "2026-04-S2" or (first.year == 2026 and first.month == 9 and sem == 1),
            "stats": {
                "analyzed_minutes": stats["analyzed_minutes"],
                "dom48_minutes": stats["dom48_minutes"],
                "dom48_hours": stats["dom48_hours"],
                "dom48_percent_of_analyzed": stats["dom48_percent_of_analyzed"],
                "band_counts": stats["band_counts"],
                "count10": stats["count10"],
                "mins10": stats["mins10"],
                "hours10": stats["hours10"],
                "count15": stats["count15"],
                "mins15": stats["mins15"],
                "hours15": stats["hours15"],
                "count30": stats["count30"],
                "mins30": stats["mins30"],
                "hours30": stats["hours30"],
                "count60": stats["count60"],
                "mins60": stats["mins60"],
                "hours60": stats["hours60"],
                "ordinance_events": stats["ordinance_events"],
                "longest_minutes": stats["longest_runs"][0]["duration_minutes"] if stats["longest_runs"] else 0,
                "longest_hours": stats["longest_runs"][0]["duration_hours"] if stats["longest_runs"] else 0.0,
            },
        })
    return rows
...
def main():
    ...
    current, _runs = summarize(data)

    reference_path = ROOT / "data" / "r6e8a_4_8_10min_full.json"
    reference = json.loads(reference_path.read_text(encoding="utf-8"))
    reference_end = datetime.fromisoformat(reference["latest_complete_analyzed_minute_utc"]) + timedelta(minutes=1)
    reference_stats, _reference_runs = summarize(subset(data, reference_end))
    reference_keys = (
        "analyzed_minutes", "dom48_minutes", "count10", "mins10",
        "count15", "mins15", "count30", "mins30", "count60", "mins60",
        "ordinance_events",
    )

    rows = semester_rollups(data)
    checks = [
        {
            "name": "1_station_scope_and_time_anchor",
            "pass": (
                START_UTC == START_ET.astimezone(timezone.utc)
                and min(data) >= int(START_UTC.timestamp())
                and max(data) < int(requested_end.timestamp())
            ),
            "observed": {
                "station": "AM.R6E8A.00",
                "channel": "HDF",
                "first_epoch": min(data) if data else None,
                "last_epoch": max(data) if data else None,
    ...
    payload = {
        ...
        "analysis_start_et": "2026-04-12T00:00:00-04:00",
        ...
        "method": {
            ...
            "channel_boundary": "HDF only; EHZ remains separate",
            "ranking_primary": "4-8 Hz dominant share of analyzed HDF minutes; duration and sustained-event burden are retained as secondary measures",
        },
        "current": current,
        "semester_rollups": rows,
        "reference_checkpoint": {
            "source": "data/r6e8a_4_8_10min_full.json",
            "end_exclusive_utc": reference_end.isoformat(),
            "expected_current": reference["current"],
